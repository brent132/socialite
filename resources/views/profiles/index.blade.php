    ...
    <div class="mt-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-1 sm:gap-3 md:gap-6">
            ...
            <div class="relative group aspect-square cursor-pointer">
                ...
                <!-- Overlay -->
                <div class="absolute inset-0 bg-black bg-opacity-40 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center pointer-events-none">
                    <div class="flex items-center space-x-6 text-white">
                        <!-- Likes -->
                        <div class="flex items-center space-x-2">
                            <span class="font-semibold">{{ $post->likes ? $post->likes->count() : 0 }}</span>
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="{{ $post->likes && $post->likes->where('user_id', auth()->id())->count() > 0 ? 'currentColor' : 'none' }}" viewBox="0 0 24 24" stroke="currentColor">
